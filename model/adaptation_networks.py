...
class FilmAdaptationNetwork(nn.Module):
    ...
    """
    def __init__(self, layer, num_maps_per_layer, num_blocks_per_layer, z_g_dim, type_block = 'basic'):
        super().__init__()
...
class FilmLayerNetwork(nn.Module):
    ...
    """
    def __init__(self, num_maps, num_blocks, z_g_dim, type_block):
        super().__init__()
        self.z_g_dim = z_g_dim
        ...
        if self.type_block == 'bottleneck':
            ...
            self.individual_layer = nn.Sequential(
                nn.Linear(self.z_g_dim, self.num_maps[2]),
                nn.ReLU()
            )
            ...
            self.gamma3_processors, self.gamma3_regularizers = torch.nn.ModuleList(), torch.nn.ParameterList()
            self.beta3_processors, self.beta3_regularizers = torch.nn.ModuleList(), torch.nn.ParameterList()

            # Generate the required layers / regularization parameters, and collect them in ModuleLists and ParameterLists
            for _ in range(self.num_blocks):
                regularizer_1 = torch.nn.init.normal_(torch.empty(num_maps[0]), 0, 0.001)
                regularizer_2 = torch.nn.init.normal_(torch.empty(num_maps[2]), 0, 0.001)

                self.gamma1_processors.append(self._make_layer(num_maps[0]))
                self.gamma1_regularizers.append(torch.nn.Parameter(regularizer_1, requires_grad=True))

                self.beta1_processors.append(self._make_layer(num_maps[0]))
                self.beta1_regularizers.append(torch.nn.Parameter(regularizer_1, requires_grad=True))

                self.gamma2_processors.append(self._make_layer(num_maps[1]))
                self.gamma2_regularizers.append(torch.nn.Parameter(regularizer_1, requires_grad=True))

                self.beta2_processors.append(self._make_layer(num_maps[1]))
                self.beta2_regularizers.append(torch.nn.Parameter(regularizer_1, requires_grad=True))
                
                self.gamma3_processors.append(self._make_layer(num_maps[2]))
                self.gamma3_regularizers.append(torch.nn.Parameter(regularizer_2, requires_grad=True))

                self.beta3_processors.append(self._make_layer(num_maps[2]))
                self.beta3_regularizers.append(torch.nn.Parameter(regularizer_2, requires_grad=True))
        else:
            # Initialize a simple shared layer for all parameter adapters (gammas and betas)
            self.shared_layer = nn.Sequential(
    ...
    def forward(self, x):
        ...
        for block in range(self.num_blocks):
            ...
            if self.type_block == 'bottleneck':
                block_param_dict['gamma3'] = self.gamma3_processors[block](x2).squeeze() * self.gamma3_regularizers[block] + torch.ones_like(self.gamma3_regularizers[block])
                block_param_dict['beta3'] = self.beta3_processors[block](x2).squeeze() * self.beta3_regularizers[block]
    ...
    def regularization_term(self):
        ...
        for gamma_regularizer, beta_regularizer in zip(self.gamma2_regularizers, self.beta2_regularizers):
            ...
            l2_term += (beta_regularizer ** 2).sum()
        if self.type_block == 'bottleneck':
            for gamma_regularizer, beta_regularizer in zip(self.gamma3_regularizers, self.beta3_regularizers):
                l2_term += (gamma_regularizer ** 2).sum()
                l2_term += (beta_regularizer ** 2).sum()
        return l2_term
...
class FilmArAdaptationNetwork(nn.Module):
    ...
    def forward(self, x, task_representation):
        ...
        z = self.feature_extractor.base_model.get_layer_output(x, None, 0)
        z_hn = flatten(z)
        ...
        for layer, hn_layer in enumerate(self.layers):
            ...
            z = self.feature_extractor.base_model.get_layer_output(z, param_dicts, layer + 1)
            z_hn = flatten(z)
        return param_dicts
