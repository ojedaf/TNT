            ...
            if (episode + 1)%num_episode_to_val == 0 or episode == 0:
                ...
                with experiment.validate():
                    ...
                    if test_accuracy > last_accuracy:
                        # save networks
                        print('Saving ... ')
                        torch.save(model.state_dict(),str(path_checkpoint_model + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot_2LOOP.pkl"))
...
def main():
    ...
    if dict_conf['train_mode']:
        ...
        metatrain_data, num_vid_train = tg.get_data_classes(path_metatrain, path_frames, conf_tsm['num_segments'], SAMPLE_NUM_PER_CLASS, BATCH_NUM_PER_CLASS, text_type)
        metaval_data, num_vid_val = tg.get_data_classes(path_metaval, path_frames, conf_tsm['num_segments'], SAMPLE_NUM_PER_CLASS, BATCH_NUM_PER_CLASS, text_type)
    ...
    with experiment.test():
        print("Init test")
        if os.path.exists(str(path_checkpoint_model + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot_2LOOP.pkl")):
            dict_param = torch.load(str(path_checkpoint_model + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot_2LOOP.pkl"))
        ...
        metatest_data, num_vid_test = tg.get_data_classes(path_metatest, path_frames, conf_tsm['num_segments'], SAMPLE_NUM_PER_CLASS, BATCH_NUM_PER_CLASS, text_type)
