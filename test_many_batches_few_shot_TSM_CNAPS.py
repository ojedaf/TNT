...
# Hyper Parameters
PATH_LIST_METRICS = dict_conf['PATH_LIST_METRICS']
...
def test(model, transform_test, meta_data, episodes, taskGen, batch_num_class, print_enable = False):
    accuracies = []

    num_total_episodes = episodes*10  #episodes_train*num_batch_train
    new_episodes = int(num_total_episodes/batch_num_class)
    print("Testing... N_episode: {}".format(new_episodes), flush=True)

    with torch.no_grad():

        task_val = taskGen(meta_data, CLASS_NUM, SAMPLE_NUM_PER_CLASS,batch_num_class, path_frames, num_snippets = conf_tsm['num_segments'], num_episodes = new_episodes)
        ...
        batch_dataloader_val = tg.get_data_loader(task_val,num_per_class=batch_num_class,split="test",shuffle=True, transforms = transform_test,  test_mode = True, workers = pool)
...
def main():
    ...
    train_augmentation = model.feature_extractor.get_augmentation(flip=False if 'something' in dataset_name or 'jester' in dataset_name else True)
    ...
    with experiment.test():
        ...
        if os.path.exists(str(path_checkpoint_model + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl")):
            dict_param = torch.load(str(path_checkpoint_model + str(CLASS_NUM) +"way_" + str(SAMPLE_NUM_PER_CLASS) +"shot.pkl"))
            model.load_state_dict(dict_param)
            print("load parameters model - to test")

        model.train()  # set encoder is always in train mode to process context data
        model.feature_extractor.eval()  # feature extractor is always in eval mode

        list_accuracy = []
        for num_batch_i in BATCH_NUM_PER_CLASS: 
            metaval_data, num_vid_val = tg.get_data_classes(path_metaval, path_frames, conf_tsm['num_segments'], SAMPLE_NUM_PER_CLASS, num_batch_i, text_type)
            print("num_vid_val: {}, num batch: {}".format(num_vid_val, num_batch_i))
            mean_accuracy, h = test(model, transform_test, metaval_data, VAL_EPISODE, taskGen, num_batch_i, True)
            ...
            dict_step = {'acc': mean_accuracy, 'desv': h, 'num_batch': num_batch_i}

            list_accuracy.append(dict_step) 
        
        path_tosv_pkl = os.path.join(PATH_LIST_METRICS, 'metrics_batch_wo_DynamicMod.pkl')
        with open(path_tosv_pkl, 'wb') as handle:
            pickle.dump(list_accuracy, handle)
